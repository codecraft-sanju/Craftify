...
const CraftifyContent = () => {
  ...
  // --- CHANGES MADE HERE: Fetch Slider Products FIRST to get ALL ids (real + random) ---
  const viewedIdsFromStorage = JSON.parse(localStorage.getItem('recentlyViewed')) || [];

  const { data: sliderProducts = [] } = useQuery({
    queryKey: ['sliderProducts', viewedIdsFromStorage],
    queryFn: async () => {
      // Make sure the endpoint exactly matches your routes
      const res = await fetch(`${API_URL}/api/products/recently-viewed`, { 
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ productIds: viewedIdsFromStorage })
      });
  ...
  // Extract all IDs (including the random ones generated by backend)
  const sliderProductIds = sliderProducts.map(p => p._id);

  // Main Products Query (Chained after slider logic)
  const { data: products = [], isLoading: productsLoading } = useQuery({
    queryKey: ['products', activeCategory, debouncedSearch, sliderProductIds],
    enabled: sliderProducts.length > 0 || viewedIdsFromStorage.length === 0, // Wait for slider to finish
    queryFn: async () => {
      ...
      if (activeCategory === 'All' && !debouncedSearch) {
          ...
          if (sliderProductIds.length > 0) {
              // Pass the complete list of 15 IDs to exclude them from grid
              url.searchParams.append('recentlyViewed', sliderProductIds.join(','));
          }
      }
    ...
    <div className="min-h-screen bg-[#FEFAEF]">
      ...
      <main className={showNavbar ? 'pt-16 md:pt-0' : ''}>
        ...
        <Route path="/" element={<AuthRedirect user={currentUser}><Navigate to="/shop" replace /></AuthRedirect>} />
          ...
          {/* --- CHANGES MADE HERE: Pass sliderProducts down to ShopView --- */}
