...
const ProductDetail = ({ addToCart, openChat, currentUser, products }) => {
  ...
  const handleAddToCart = () => {
      const customization = product.customizationAvailable && customText ? { text: customText, font: customFont } : null;
       ...
       <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
         <div className="space-y-6">
             {product.customizationAvailable ? (
                 <LiveCustomizer product={product} customText={customText} setCustomText={setCustomText} customFont={customFont} setCustomFont={setCustomFont}/>
             ) : (
                <div className="aspect-[4/3] bg-slate-100 rounded-3xl overflow-hidden shadow-sm border border-slate-100">
         ...
         <div className="flex flex-col h-full">
             ...
             <div className="flex flex-col gap-3 mt-auto">
                <Button size="lg" onClick={handleAddToCart} className="w-full" disabled={product.stock <= 0}>
...
const CraftifyContent = () => {
  ...
  const handleCheckout = async () => {
    ...
    try {
        ...
        console.error(error);
        addToast("Error", "Could not place order", "error");
    }
  };

  const showNavbar = location.pathname !== '/founder' && location.pathname !== '/seller-register' && location.pathname !== '/seller-login' && location.pathname !== '/admin-login';
    ...
    <div className="bg-[#F8FAFC] min-h-screen font-sans text-slate-900 selection:bg-indigo-200 pb-20 md:pb-0">
      ...
      <main className="min-h-screen">
         <Routes>
             ...
             <Route path="/shop" element={<ShopView activeCategory={activeCategory} setActiveCategory={setActiveCategory} searchQuery={searchQuery} setSearchQuery={setSearchQuery} addToCart={addToCart} products={products} shops={[]} isLoading={productsLoading} />} />
             <Route path="/product/:id" element={<ProductDetail addToCart={addToCart} openChat={openCustomizationChat} currentUser={currentUser} products={products} />} />
